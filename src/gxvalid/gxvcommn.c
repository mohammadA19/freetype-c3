...
#define FT_COMPONENT  gxvcommon
                                   ...
                                   GXV_Validator  gxvalid )
  {
    ...
    ft_qsort( buff, ( nmemb + 1 ), sizeof ( ushort ),
    ...
    for ( i = 0; i < nmemb; i++ )
    {
      ...
      *(length[i]) = (ushort)( buff[j + 1] - buff[j] );
                                       ...
                                       GXV_Validator       gxvalid )
  {
    ...
    entrySelector--;
    searchRange = (ushort)( searchRange * binSrchHeader->unitSize );
    rangeShift  = (ushort)( binSrchHeader->nUnits * binSrchHeader->unitSize
                               - searchRange );
...
#define GXV_UNITSIZE_VALIDATE( FORMAT, UNITSIZE, NUNITS, CORRECTSIZE ) \
                                 ...
                                 GXV_Validator  gxvalid )
  {
    ...
    for ( unit = 0, gid = 0; unit < nUnits; unit++ )
    {
      ...
      for ( gid = firstGlyph; gid <= lastGlyph; gid++ )
      {
        value = gxvalid->lookupfmt4_trans( (ushort)( gid - firstGlyph ),
                                         &base_value,
                                         limit,
                                         gxvalid );
                                 ...
                                 GXV_Validator  gxvalid )
  {
    ...
    gxv_glyphid_validate( firstGlyph, gxvalid );
    gxv_glyphid_validate( (ushort)( firstGlyph + glyphCount ), gxvalid );
    ...
    for ( i = 0; i < glyphCount; i++ )
    {
      GXV_LIMIT_CHECK( 2 );
      value = GXV_LOOKUP_VALUE_LOAD( p, gxvalid->lookupval_sign );
      gxvalid->lookupval_func( (ushort)( firstGlyph + i ), &value, gxvalid );
    }
                         ...
                         GXV_Validator  gxvalid )
  {
    ...
    n_points = (ushort)outline.n_points;
                           ...
                           GXV_Validator  gxvalid )
  {
    ...
    *maxClassID_p = 3;  /* Classes 0, 2, and 3 are predefined */
    ...
    gxv_glyphid_validate( (ushort)( firstGlyph + nGlyphs ), gxvalid );
                           ...
                           GXV_Validator  gxvalid )
  {
    ...
    FT_Byte   entry;
    FT_Byte   state;
    int    entrySize = 2 + 2 + GXV_GLYPHOFFSET_SIZE( statetable );
  ...
  {
    ushort   stateSize;
    ushort   classTable;     /* offset to Class(Sub)Table */
    ushort   stateArray;     /* offset to StateArray */
    ushort   entryTable;     /* offset to EntryTable */

    ushort   classTable_length;
    ushort   stateArray_length;
    ushort   entryTable_length;
    FT_Byte     maxClassID;
    FT_Byte     maxState;
    FT_Byte     maxEntry;
  ...
  static void
  gxv_XClassTable_lookupval_validate( ushort            glyph,
  ...
  */
  static GXV_LookupValueDesc
  gxv_XClassTable_lookupfmt4_transit( ushort            relative_gindex,
    ...
    *maxEntry_p = 0;
    ...
    while ( p + ( ( 1 + maxClassID ) * 2 ) <= limit )
    {
      ...
      for ( clazz = 0; clazz <= maxClassID; clazz++ )
      {
        ...
        *maxEntry_p = (ushort)FT_MAX( *maxEntry_p, entry );
      }
    }
    GXV_TRACE(( "parsed: maxState=%d, maxEntry=%d\n",
                *maxState_p, *maxEntry_p ));
                            ...
                            GXV_Validator  gxvalid )
  {
    ...
    ushort  entry;
    ushort  state;
    int     entrySize = 2 + 2 + GXV_GLYPHOFFSET_SIZE( xstatetable );
    ...
    for (entry = 0; entry <= maxEntry; entry++ )
    {
      ...
      state = (ushort)( newState_idx / ( 1 + maxClassID ) );
                            ...
                            GXV_Validator  gxvalid )
  {
    ...
    else
    {
      ...
      gxvalid->xstatetable.maxClassID =
        (ushort)( gxvalid->xstatetable.nClasses - 1 );
    }
