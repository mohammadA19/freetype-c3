...
#define GXV_OPBD_DATA( FIELD )  GXV_TABLE_DATA( opbd, FIELD )
  ...
  static void
  gxv_opbd_LookupValue_validate( ushort            glyph,
                                 ...
                                 GXV_Validator        gxvalid )
  {
    ...
    for ( i = 0; i < 4; i++ )
    {
      ...
      if ( GXV_OPBD_DATA( format ) )    /* format 1, value is ctrl pt. */
      {
        ...
        gxv_ctlPoint_validate( glyph, (ushort)delta_value, gxvalid );
      }
  ...
  /*
    ...
    +===============+         |             +
    | lastGlyph[1]  |         | (glyphID - firstGlyph) * 4 * sizeof(short) [byte]
    +---------------+         |
    | firstGlyph[1] |         |
    +---------------+         |
    | offset[1]     |         |
  ...
  gxv_opbd_LookupFmt4_transit( ushort            relative_gindex,
                               ...
                               GXV_Validator        gxvalid )
  {
    ...
    /* XXX: check range? */
    value.u = (ushort)( base_value_p->u +
                           relative_gindex * 4 * sizeof ( short ) );
